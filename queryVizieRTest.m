classdef queryVizieRTest < matlab.unittest.TestCase
    ...
    methods (Test)
        function query_urat1(test)
            target = queryVizieR(10.684708,41.232,2,4);
            target=target.get_urat1();
            test.verifyEqual(target.getitem('RA',5),10.6821147);
            ...
            test.verifyEqual(target.getitem('URAT1',7),{'657-010472'});
        end
        ...
        function query_ppmxl(test)
            ...
            test.verifyEqual(target.getitem('RA',1020),72.844548);
            test.verifyEqual(target.getitem('DE',1000),43.683070);
            test.verifyEqual(target.getitem('Jmag',954),15.863);
            test.verifyEqual(target.getitem('pmRA',101),-7.5);
            test.verifyEqual(target.getitem('f1',124),1);
        end
    end
